...
spark = SparkSession.builder.appName("BankLoan").enableHiveSupport().getOrCreate()

# Step 2: Load the data from the Hive table 'bank_loan_data' into a Spark DataFrame
bank_loan_df = spark.sql("SELECT Age, Income, CreditScore, LoanAmount FROM bank_loan_data")

# Step 3: Handle null values by either dropping or filling them
bank_loan_df = bank_loan_df.na.drop()  # Drop rows with null values

# Step 4: Prepare the data for MLlib by assembling features into a vector
assembler = VectorAssembler(
    inputCols=["Age", "Income", "CreditScore"],
    outputCol="features",
    handleInvalid="skip"  # Skip rows with null values
)
assembled_df = assembler.transform(bank_loan_df).select("features", "LoanAmount")
